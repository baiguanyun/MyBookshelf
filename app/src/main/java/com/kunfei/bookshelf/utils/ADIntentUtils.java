...
public class ADIntentUtils {
    ...
    private boolean shouldOverrideUrlLoadingByAppInternal(WebView view, String url, boolean interceptExternalProtocol) {
        ...
        if (matchingActivities.size() == 1) {
            ...
            if (externalAppTitle.toString().contains("京东") || externalAppTitle.toString().contains("淘宝")
                    || externalAppTitle.toString().contains("百度") || externalAppTitle.toString().contains("微博")
                    || externalAppTitle.toString().contains("飞猪") || externalAppTitle.toString().contains("携程")
                    || externalAppTitle.toString().contains("58同城")) {
                return true;
            }
            AlertDialog.Builder builder = new AlertDialog.Builder(mActivity);
            builder.setTitle("网页请求打开" + externalAppTitle + "应用，立即打开？");
